...
We also add the line `'conduit.articles',`{.verbatim} to INSTALLED~APPS~
in `settings.py`{.verbatim}:
...
What we\'re doing here is take the `AbstractUser`{.verbatim} model and
...
The `templates/article_preview.html`{.verbatim} file should look like
this:
...
Then, we modify the `articles/urls.py`{.verbatim} file:
...
Notice that the `LoginRequiredMixin`{.verbatim} should be at the
...
In `users/views.py`{.verbatim}, we make sure that editing or deleting
...
In the `templates`{.verbatim} folder, create
...
In `templates/article_favorite.html`{.verbatim}:
...
Checking if an article is in a user\'s `favorites`{.verbatim} should be
done in the view (or, even better, the model) instead of the template,
but we would have to change our templates\' structure and write new
views if we wanted to include a `Favorite`{.verbatim} button in the
`article_preview.html`{.verbatim} template.

### ArticlePreviewView

In `templates/article_preview.html`{.verbatim}:
...
1.  [TODO]{.todo .TODO} ArticleFavoriteView success~url~

### Feeds

Now, we need to do something with this `Favorite`{.verbatim} feature:
we\'ll display a user\'s favorited articles on their profile.
...
First, we create the `Tag`{.verbatim} object itself.

In `articles/models.py`{.verbatim}:
...
The `tag`{.verbatim} field will contain the tag itself, while
`slug`{.verbatim} is just a way for us to retrieve it more easily.

Now, we need a way to assign tags to an article: we can achieve this by
wadding a `ForeignKey`{.verbatim} to the `Article`{.verbatim} model. In
`articles/models.py`{.verbatim}:
...
We have modified the models: we need to `makemigrations`{.verbatim} and
`migrate`{.verbatim}.
...
#+attr_org: :width 400
```
[file:./Screenshots/editor - tags.mp4](./Screenshots/editor - tags.mp4)
...
We create `templates/article_tags.html`{.verbatim}:

``` html
```

We include this new template into `templates/editor.html`{.verbatim}:
