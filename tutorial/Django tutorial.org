            ...
            {% if user.is_authenticated %}                          <!-- new from here -->
                ...
                {% comment %}
                    <li class="nav-item">
                        <a
                            rel="prefetch"
                            href="{% url 'settings' username=user.username %}"
                            class="nav-link">
...
Let's create a new user by clicking on =Sign up= in our navbar. Enter a username, an email (which doesn't have to be a real one as long as it's the right format), and a password. When you finalise this action by clicking the button =Sign up=, you'll notice that you're redirected to the homepage without being logged in. This is fine - you can sign in manually as the user you just created or you might want to implement an email verification before allowing sign ups - but, in our case, we might as well sign in the user automatically.

In =users/views.py=, add the following to =SignUpView= (as explained in [[https://stackoverflow.com/questions/3222549/how-to-automatically-login-a-user-after-registration-in-django/28974691#28974691][this StackOverflow answer]]):
...
class SignUpView(CreateView):
    ...
    def form_valid(self, form):                     # new
        valid = super().form_valid(form)            # new
        login(self.request, self.object)            # new
        return valid                                # new
#+end_src
