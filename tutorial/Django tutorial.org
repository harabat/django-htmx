...
class ArticleDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context["has_favorited"] = self.request.user.profile.has_favorited(   # new
                self.object                                                     # new
            )                                                                   # new
        return context
#+end_src
...
>
    ...
    >
        <span class="ion-heart">
            {% if has_favorited %}Unfavorite{% else %}Favorite{% endif %} Article ({{ article.favorited_by.count }})
        </span>
    </button>
</form>
#+end_src
*** TODO ArticlePreviewView
In =articles/article_preview.html=:
