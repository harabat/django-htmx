...
Try creating a new user: this should have resolved the issue.
*** TODO check why =authenticate= returs =None=
#+begin_src python
class SignUpView(CreateView):
    model = User
    fields = ["username", "email", "password"]
...
#+end_src
[[https://stackoverflow.com/questions/3222549/how-to-automatically-login-a-user-after-registration-in-django/70582911#70582911][How to automatically login a user after registration in django - Stack Overflow]]
** Viewing profiles
...
#+ATTR_ORG: :width 600
[[./Screenshots/Profile - ProfileDetailView error.png]]
...
Let's try again: we should see an actual profile page (though there isn't much on it yet).
** Editing profiles
